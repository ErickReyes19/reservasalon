<x-app-layout>
    <div class="row justify-content-center ">
        <div class="col-md-6 shadow p-3 mb-5 bg-body-tertiary rounded my-4">
            <div class="container mt-5">
                <form id="edittipoaccesorio" method="post">
                    @csrf
                    <div class="form-group">
                        <label for="competencia">Tipo Equipo:</label>
                        <input type="text" class="form-control" id="nombre" name="nombre" value="{{$tipoAccesorio->nombre}}">
                    </div>
                    <div class="form-group mt-3">
                        <div class="form-check form-switch">
                            <input type="hidden" name="estado" value="0"> <!-- Campo oculto con valor predeterminado de 0 -->
                            <input class="form-check-input" type="checkbox" id="estado" name="estado" value="1" {{$tipoAccesorio->estado ? 'checked' : ''}}>
    ...
    <script>
        $(document).ready(function() {
            $('#edittipoaccesorio').submit(function(e) {
                e.preventDefault();
                var formData = $(this).serialize();
                $.ajax({
                    type: 'POST',
                    url: "{{ route('tipoaccesorio.update', $tipoAccesorio->id) }}",
                    data: formData,
